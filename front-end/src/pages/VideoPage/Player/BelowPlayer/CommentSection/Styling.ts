// noinspection CssUnknownUnit,CssUnresolvedCustomProperty,CssUnusedSymbol,CssNonIntegerLengthInPixels
...
export const CommentBody = styled(Stack)<ComponentWithProps>`
  border-radius: 4px;
  position: relative;
  ...
  margin-bottom: ${({ isReply }) => (isReply ? "8px" : "0")};
`;

export const AuthorAvatarContainer = styled("div")``;

export const AuthorAvatar = styled(Avatar)<ComponentWithProps>`
  margin-right: 16px;
  width: ${({ isReply }) => (isReply ? "24px" : "40px")};
  height: ${({ isReply }) => (isReply ? "24px" : "40px")};
  cursor: pointer;
`;
...
export const CommentContentInnerContainer = styled("div")<ComponentWithProps>`
  overflow: auto;
  width: 100%;
